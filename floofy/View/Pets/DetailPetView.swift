    ...
    var body: some View {
        NavigationStack {
            ScrollView(showsIndicators: true) {
                VStack(spacing: 15) {
                    ZStack {
                        Image(uiImage: UIImage(data: selectionPet.imagePets ?? Data()) ?? UIImage())
                            .resizable()
                                    ...
                                    .overlay(
                                        List {
                                            ForEach(selectionPet.activityArray) { activityPet in
                                                HStack {
                                                    Image(uiImage: UIImage(data: activityPet.imageGrooming ?? Data()) ?? UIImage())
                                                        .resizable()
                                                    ...
                                                    VStack(alignment: .leading) {
                                                        Text("Grooming")
                                                            ...
                                                            .foregroundColor(.black)
                                                        Text(activityPet.descriptionGrooming ?? "Unknown")
                                                            .font(.system(size: 15, weight: .regular))
                                                            .foregroundColor(.black)
                                                    }
                                                    Spacer()
                                                    Text(activityPet.dateGrooming ?? Date(), style: .date)
